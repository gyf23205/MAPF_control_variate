...
from getpath_new_allocation import load_data, PathPlanningCNN
...
def run_command(command):
    try:
        completed_process = subprocess.run(command, shell=True, text=True, capture_output=True, encoding='utf-8')
...
def tsp_solver(targets, obstacles, starts, result, start_map, pi):
    ...
    base_file_path = "tsp_solver/cpp-do/test/map/output_map_"
    header = f"type octile\nheight {map_size_x}\nwidth {map_size_y}\nmap\n"

    ensure_directory_exists("tsp_solver/cpp-do/test/map")
    ...
    base_file_path = "tsp_solver/cpp-do/test/map_input"
    ...
    actualcost = []
    commands = [
        f"./tsp_solver/cpp-do/build/api ./tsp_solver/cpp-do/test/map/output_map_{i + 1}.map ./tsp_solver/cpp-do/test/map_input/input_agent{j}_batch{i}.json ./tsp_solver/cpp-do/test/output/ans_batch{i}_agent{j}.txt"
    ...
    for i in range(batchsize):
        total_cost = 0
        for j in range(agent_n):
            output_file_path = f'tsp_solver/cpp-do/test/output/ans_batch{i}_agent{j}.txt'
